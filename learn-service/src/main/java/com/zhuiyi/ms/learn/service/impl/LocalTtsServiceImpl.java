...
@Service
@Slf4j
public class LocalTtsServiceImpl implements IAudioTtsService {
    ...
    @Override
    public ReqAudioTtsDTO getAudioTts(Object object) throws Exception {
        ReqAudioTtsDTO reqAudioTtsDTO = (ReqAudioTtsDTO) object;
        Map<String, Object> params = new HashMap<>();
        ...
        if (StringUtils.isNotEmpty(reqAudioTtsDTO.getText())) {
            ...
            if (null != audioTtsEntity && StringUtils.isNotEmpty(reqAudioTtsDTO.getText())) {
                if (StringUtils.isNotEmpty(audioTtsEntity.getText())
                        && audioTtsEntity.getText().equals(reqAudioTtsDTO.getText())) {
                    // 读取txt文件
    ...
    /**
     * @Description: 删除t_exam_tts:批量软删除 @param ${tags} @return
     *               ${return_type} @throws @author kloazhang @date 2019/5/29 11:25
     */
    @Transactional(rollbackFor = Exception.class)
    public int deleteExamTtsRecord(ReqDeleteTtsDTO reqDeleteTtsDTO) {

        int result = 0;
        if (null != reqDeleteTtsDTO && null != reqDeleteTtsDTO.getExamId()) {
            Map<String, Object> params = new HashMap<>();
            ...
            if (CollectionUtils.isNotEmpty(reqDeleteTtsDTO.getTtsTexts())) {
                for (TtsReqDTO ttsReqDTO : reqDeleteTtsDTO.getTtsTexts()) {
                    textIds.add(ttsReqDTO.getTextId());
                }
    ...
    public void writeFile(String path, String fileName, byte[] content) throws IOException {
        try {
